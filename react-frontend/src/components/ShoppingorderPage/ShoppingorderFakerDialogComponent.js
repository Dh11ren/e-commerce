...
import shoppingorderFakerFactory from "./shoppingorderFakerFactory";
...
const ShoppingorderFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = shoppingorderFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("shoppingorder").create(elem));
...
export default connect(null, mapDispatch)(ShoppingorderFakerDialogComponent);
