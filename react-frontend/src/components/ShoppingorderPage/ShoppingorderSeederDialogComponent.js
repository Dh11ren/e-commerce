...
import seederData from "./shoppingorder.seeder.json";
...
const ShoppingorderSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("shoppingorder").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(ShoppingorderSeederDialogComponent);
